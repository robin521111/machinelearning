...
def weight_variable(shape, dtype, name):
    initial = tf.truncated_normal(
        shape=shape, stddev=0.1, dtype=dtype, name=name)
...
weight_conv1 = weight_variable(
    [5, 5, 1, 32], dtype="float", name='weight_conv1')
...
# convolution 2
weight_conv2 = weight_variable(
    [5, 5, 32, 64], dtype="float", name='weight_conv2')
...
weight_fc1 = weight_variable(
    [7 * 7 * 64, 1024], dtype="float", name='weight_fc1')
