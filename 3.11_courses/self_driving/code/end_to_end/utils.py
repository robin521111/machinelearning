...
def visualize(epoch_id, machine_steering, out_dir, perform_smoothing=False,
    ...
    out_path = join_dir(out_dir, 'epoch{:0>2}_human_machine.avi'.format(epoch_id))

    # vw = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc(*'X264' ), 30, vid_size)

    # old
    #vw = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc('X','2','6','4'), 30, vid_size)    vw = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc('X','2','6','4'), 30, vid_size)

    #vw = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc('M','J','P','G'), 30, vid_size)
    vw = cv2.VideoWriter(out_path, 0, 30, vid_size)
